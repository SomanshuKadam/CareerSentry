...
export function JobsView({ jobs = demoJobs, sourceLabel = "Saved RevRag recovery evidence" }: { jobs?: DemoJob[]; sourceLabel?: string }) {
    ...
    <>
      <div className="filter-bar">
        <div className="filter-left">
          ...
          {stateFilters.map((filter) => <span key={filter.value} onClick={() => setState(filter.value)}><FilterChip active={state === filter.value}>{filter.label}{filter.value === "all" ? <span className="table-count">{jobs.length}</span> : null}</FilterChip></span>)}
        </div>
      ...
      <p className="footer-note">{filteredJobs.length} roles from {sourceLabel} · Official role pages are shown for provenance review; application routes are not collected.</p>
    </>
  );
}
